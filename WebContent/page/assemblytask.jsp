...
<body>
			...
			<div class="cl pd-20">
				<div class="cl pd-5 bg-1 bk-gray">
					<span class="l"> <a href="javascript:;" onclick="delBatch()"
						...
						onclick="assemblytask_add('添加装配任务','${APP_PATH}/assebmlytask_add.jsp','900','450')"><i
							class="Hui-iconfont">&#xe600;</i> 添加装配任务</a>
					</span>
				...
				<div class="mt-10">
					<table
						...
						<thead>
							<tr>
								<th scope="col" colspan="9">装配任务管理</th>
							</tr>
							<tr class="text-c">
								<th width="25"><input type="checkbox" value="" name=""></th>
								<th width="40">任务书编号</th>
								<th width="100">成品类型</th>
	...
	<script type="text/javascript">
		...
		function toPage(pn) {
            $.ajax({
                url:"${APP_PATH}/assemblytask_list",
                data:"pn="+pn,
				type:"GET",
        ...
        function build_table(result) {
            ...
            $.each(assems, function (index, item) {
				...
				var finishedTypeEntityType=$("<td></td>").append(item.finishedTypeEntity.productType);
				var leaderName=$("<td></td>").append(item.assemblyLeaderEntity.name);
				var partformulaEntityName=$("<td></td>").append(item.partformulaEntity.formulaName);
				...
				var a1=$("<a></a>").attr("title","编辑").attr("href","javascript:;")
					.attr("onclick","asseblytask_edit('编辑装配任务','${APP_PATH}/assemblytask_modify.jsp',"+item.id+",'800','400')")
                ...
                var a2=$("<a></a>").addClass("ml-5").attr("title","删除").attr("href","javascript:;")
                    .attr("onclick","asseblytask_del("+item.id+")")
                    .attr("style","text-decoration: none")
                    .append(
                        $("<i></i>").addClass("Hui-iconfont").append("&#xe6e2;")
                    );
                ...
                tr.append(check);
                tr.append(id);
                tr.append(finishedTypeEntityType);
		...
		function assemblytask_add(title, url, w, h) {
		 ...
		 /*装配任务-编辑*/
		function asseblytask_edit(title, url, id, w, h) {
			layer_show(title,url+"?id="+id,w,h);
		}


		/*装配任务-单个删除*/
		function asseblytask_del(id) {
        ...
        function sendAjaxDel(ids) {
            $.ajax({
                url: "${APP_PATH}/assemblytask_del/" + ids,
                type: "DELETE",
                success: function (result) {
