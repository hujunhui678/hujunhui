...
<body>
	...
	<form action="#" method="post" class="form form-horizontal"
		id="form-productivetask-modi" style="line-height: 30px;">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<span class="select-box" style="width: 100px; margin-left: 8.95px;">
					<select class="select" name="partTypeEntity" size="1" id="partTypeEntity">
				</select>
				</span>
			</div>
		</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" id="productionNum">
			</div>
		</div>
	...
	<script type="text/javascript">
        ...
        $.ajax({
			//从上个页面传来的id
			url:"${APP_PATH}/productivetask1?id=${param.id}",
			type:"GET",
			success:function (result) {
			    debugger
                var productivetask=result.extend.productivetask;
                var planId=productivetask.planId;
                $("#planId").val(planId);
			    var productionType=productivetask.partTypeEntity.partType;
			    $("#partTypeEntity").val(productionType);
                var productionLeader=productivetask.productionLeaderEntity.name;
                $("#productionLeader").val(productionLeader);
			    var productionNum=productivetask.productionNum;
			    $("#productionNum").val(productionNum);
                var dateStr=productivetask.dateStr;
                $("#estimatedTimeOfCompletion").val(dateStr);
                var desc=productivetask.desc;
				$("#desc").val(desc);
            }
		});
		...
		$("#sub").click(function() {
             ...
             */
            $.ajax({
                url:"${APP_PATH}/productivetask_edit/id="+${param.id},
                type:"PUT",
                data:$("#form-productivetask-modi").serialize(),
                success:function (result) {
