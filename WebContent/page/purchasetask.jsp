 ...
 $(function() { 
	...
	$("[xiangxi='openDesc']").click(function(){
		...
		$.post("getPurchaseOrderDesc?orderId="+orderId,null,function(data){
			...
			if(data[0].purchaseOrder.receiveAdmin){
				$("#lingyongren").html("<label class='col-sm-2 control-label no-padding-right' for='form-field-1'>领用人 </label>");
				$("#lingyongren").append("<div class='col-sm-9'><input type='text'   class='col-xs-10 col-sm-11' readonly id='receiveAdmin' style='margin-left:0px;' value='"+data[0].purchaseOrder.receiveAdmin.name+"'></div>");
			}
			if(data[0].purchaseOrder.leadingDepartment){
				$("#lingyongbumen").html("<label class='col-sm-2 control-label no-padding-right' for='form-field-1'>领用部门 </label>")
				$("#lingyongbumen").append("<div class='col-sm-9'><input type='text'   class='col-xs-10 col-sm-11' readonly id='leadingDepartment' style='margin-left:0px;' value='"+data[0].purchaseOrder.leadingDepartment.deptName+"'></div>");
			}
			},"json");
		layer.open({
	        type: 1,
